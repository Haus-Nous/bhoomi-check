...
export async function GET(request: Request, { params }: { params: Promise<{ caseId: string }> }) { const { caseId } = await params; const locale = request.headers.get("x-bhoomi-locale") === "hi" ? "hi" : "en"; documentApplicationService.ensureSeedDocuments(); const detail = caseApplicationService.getCaseDetail(caseId); if (!detail) return NextResponse.json({ error: { code: "CASE_NOT_FOUND", message: "This synthetic case was not found." } }, { status: 404 }); verificationService.run(caseId); const refreshed = caseApplicationService.getCaseDetail(caseId)!; const guidance = guidanceService.build(caseId, refreshed.verification, refreshed.documents, locale); const packets = reviewPacketService.list(caseId); return NextResponse.json({ data: { ...refreshed, guidance, reviewPackets: packets, timeline: buildTimeline({ ...refreshed, guidance }, packets) } }); }
