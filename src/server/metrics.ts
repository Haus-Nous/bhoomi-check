...
export async function measureAsync<T>(sink: MetricSink, operation: MetricEvent["operation"], work: () => Promise<T>, metadata?: MetricEvent["metadata"]): Promise<T> { const started = performance.now(); try { const value = await work(); observe(sink, { operation, outcome: "success", durationMs: performance.now() - started, metadata }); return value; } catch (error) { observe(sink, { operation, outcome: "failure", durationMs: performance.now() - started, metadata }); throw error; } }
