...
export class OpenAIExtractionProvider implements ExtractionProvider {
  ...
  async extract(document: PreparedDocument) {
    const response = await this.client.responses.create({ model: this.model, input: [{ role: "system", content: "Extract only explicitly stated facts from this synthetic land-record text. Do not infer ownership, entitlement, legal validity, discrepancies, or missing values. Every fact must quote an exact substring and provide its zero-based start/end offsets. Confidence means extraction clarity only. Mark all suggestions for human review. Use an empty uncertainty string when none is apparent." }, { role: "user", content: document.text }], text: { format: { type: "json_schema", name: "document_extraction", strict: true, schema: { type: "object", additionalProperties: false, properties: { documentType: { type: "string" }, facts: { type: "array", maxItems: 50, items: { type: "object", additionalProperties: false, properties: { key: { type: "string", enum: factKeys }, value: { type: "string" }, confidence: { type: "string", enum: ["low", "medium", "high"] }, evidence: { type: "object", additionalProperties: false, properties: { quote: { type: "string" }, start: { type: "integer", minimum: 0 }, end: { type: "integer", minimum: 1 } }, required: ["quote", "start", "end"] }, needsHumanReview: { type: "boolean" }, uncertainty: { type: "string" } }, required: ["key", "value", "confidence", "evidence", "needsHumanReview", "uncertainty"] } } }, required: ["documentType", "facts"] } } } });
...
export class ExtractionService {
  ...
  async extract(document: PreparedDocument): Promise<DocumentExtraction> { const createdAt = new Date().toISOString(); try { const raw = await this.provider.extract(document); const result = validateExtraction(raw, document.text); const extraction: DocumentExtraction = { id: crypto.randomUUID(), caseId: document.caseId, documentId: document.documentId, status: "completed", result, provider: this.provider.name, model: this.provider.model, createdAt }; this.repository.save(extraction); return extraction; } catch (error) { const extraction: DocumentExtraction = { id: crypto.randomUUID(), caseId: document.caseId, documentId: document.documentId, status: "failed", provider: this.provider.name, model: this.provider.model, createdAt, error: { code: error instanceof ExtractionValidationError ? "INVALID_MODEL_OUTPUT" : "PROVIDER_FAILURE", message: error instanceof ExtractionValidationError ? error.message : "AI extraction failed. No fields were accepted." } }; this.repository.save(extraction); return extraction; } }
}
