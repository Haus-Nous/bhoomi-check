...
export function ParcelIntelligencePanel({ caseId }: { caseId: string }) {
  ...
  return <div className="parcel-intelligence">
    ...
    <section className="parcel-details" aria-label={translated.identity}><div><p className="eyebrow">{translated.identity}</p><h2>{translated.mappedParcel}</h2><dl><div><dt>Khata (खाता)</dt><dd>{parcel.khata}</dd></div><div><dt>Khesra (खेसरा)</dt><dd>{parcel.khesra || translated.notAvailable}</dd></div><div><dt>Mauza (मौजा)</dt><dd>{parcel.mauza}</dd></div><div><dt>{translated.circle}</dt><dd>{parcel.circle}</dd></div><div><dt>{translated.district}</dt><dd>{parcel.district}</dd></div></dl></div>{geometry ? <div><p className="eyebrow">{translated.geometry}</p><h2>{geometry.geometryType}</h2><dl><div><dt>{translated.source}</dt><dd>{geometry.sourceReference}</dd></div><div><dt>{translated.provenance}</dt><dd>{geometry.provenance}</dd></div><div><dt>{translated.calculatedArea}</dt><dd>{translated.calculatedFromGeometry}</dd></div></dl></div> : <div><p className="eyebrow">{translated.geometry}</p><h2>{translated.notAvailable}</h2><p>{translated.emptyDetail}</p></div>}</section>
    ...
    <aside className="parcel-safety"><strong>{translated.safetyTitle}</strong><p>{translated.safetyDetail}</p></aside>
  </div>;
}
